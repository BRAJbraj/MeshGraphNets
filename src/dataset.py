...
from src.common import NodeType
...
def add_targets(ds, fields, add_history=True):
  """Adds target and history fields for velocity calculation."""
  def fn(trajectory):
    out = {}
    for key, val in trajectory.items():
      if key not in fields:
        out[key] = val[1:-1] if add_history else val[:-1]
        continue

      if add_history:
        out[key] = val[1:-1]              # Current (t)
        out['prev|'+key] = val[0:-2]      # Previous (t-1)
        out['target|'+key] = val[2:]      # Target (t+1)
      else:
        out[key] = val[:-1]
...
def split_and_preprocess(ds, noise_field, noise_scale, noise_gamma):
  ...
  def add_noise(frame):
    ...
    # Mask noise: Only apply to NORMAL (Metal) nodes
...
def batch_dataset(ds, batch_size):
  """Batches datasets with graph connectivity."""
